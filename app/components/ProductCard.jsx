...
export default function ServiceCard({ service, addToCart }) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-amber-900/30 bg-gradient-to-br from-gray-900 to-black hover:border-gray-500 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-amber-900/20">
      ...
      */}
      ...
      <div className="relative p-6 z-10 h-full flex flex-col">
        ...
        <div className='z-10 mt-80'>
            {/* Service Name */}
        <h3 className="z-10 text-xl font-serif font-semibold text-amber-100 mb-2">{service.name}</h3>
        
        {/* Description */}
        <p className="z-10 text-amber-200/90 text-sm mb-4">{service.description}</p>
        
        {/* Duration */}
        <div className="z-10 flex items-center gap-2 text-amber-300 mb-4">
          <Clock size={14} />
          <span className="text-sm">{service.duration}</span>
        </div>
        
        {/* Features */}
        <ul className="z-10 space-y-2 mb-6 flex-grow">
        ...
        <div className="z-10 flex items-center justify-between pt-4 border-t border-gray-500/30 mt-auto">
          <div>
          ...
          <button 
            onClick={addToCart}
            className="px-5 py-2 bg-gradient-to-r from-yellow-600 to-amber-600 text-amber-50 font-medium rounded-full hover:from-amber-600 hover:to-yellow-800 transition-all duration-300 text-sm hover:shadow-lg hover:shadow-amber-900/30"
          >
            Book Now
          </button>
        </div>
